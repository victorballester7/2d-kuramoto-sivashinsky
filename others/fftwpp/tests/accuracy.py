...
def findError(m, L, T=1, D=1, I=1):
    cmd = ["hybridconv", f"-L{L}", f"-M{2*L}",
           f"-m{m}", f"-T{T}", f"-I{I}", f"-D{D}", "-a"]
    vp = subprocess.Popen(cmd, stdout=subprocess.PIPE,
...
def getArgs():
    parser = argparse.ArgumentParser(
        description="Test accuracy of 1D complex hybrid ed convolutions. Checks powers of 2.")
    parser.add_argument(
        "-a", help="Start (exponent on 2). Default is 6.", default=6)
    parser.add_argument(
        "-b", help="Stop (exponent on 2). Default is 12.", default=12)
