...
int rk78(double *t, double x[], double *h,
         double hmin, double hmax, double tol, int maxRepetitions,
  ...
  if (count == maxRepetitions) return 1;  // Error
...
int flow(double *t, double x[], double *h, double T, double hmin, double hmax, double tol, int maxNumSteps, int maxRepetitionsRK78, int n, int (*field)(int n, double t, double x[], double f[], void *param), void *param) {
  double t0 = *t;
  int count = 0;
  // printf("holaflow-1, t = %g, h = %g, T = %g, hmin = %g, hmax = %g, tol = %g, n = %i, maxNumSteps= %d\n", *t, *h, T, hmin, hmax, tol, n, maxNumSteps);

  if (fabs(T) < tol) {
    *t = t0 + T;
    return 0;
  }
  while (fabs(*t - t0) + tol < fabs(T) && count < maxNumSteps) {
    // printf("holaflow0\n");
    if (fabs(*t + *h - t0) > fabs(T)) {
      *h = t0 + T - *t;
      // printf("holaflow0.5, t = %g, h = %g, hmin = %g, hmax = %g, tol = %g, n = %i\n", *t, *h, hmin, hmax, tol, n);
      if (rk78(t, x, h, hmin, hmax, tol, maxRepetitionsRK78, n, field, param))
        return 1;
      else
        break;
    }
    // printf("holaflow1, t = %g, h = %g, hmin = %g, hmax = %g, tol = %g, n = %i\n", *t, *h, hmin, hmax, tol, n);
    // printf("flow: x[1] = %g, x[2] = %g\n", x[0], x[1]);

    if (rk78(t, x, h, hmin, hmax, tol, maxRepetitionsRK78, n, field, param)) return 1;

    // printf("holaflow1.5, t = %g, h = %g, hmin = %g, hmax = %g, tol = %g, n = %i\n", *t, *h, hmin, hmax, tol, n);
    count++;
  }
  // printf("holaflow2\n");
